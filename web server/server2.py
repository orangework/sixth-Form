    ...
    if(request.method == "POST"):
        ...
        with con:
            ...
            if(not row):##incorrect
                ...
                return redirect("/?error=Incorrect%20username%20or%20password")
            else:
...
def teachersPage():
    ...
    if(session.get('loggedin')):
        try:
            ...
            if(len(e)<1):
                e=""
            return render_template("connect/teachers.html",teachers=getTeachers(),last=getLastTeacherUpdate()[0],error=e)
        except:
...
def commandControl():
    if(session.get("loggedin") and request.method == "POST"):
        command = request.form["com"]
        print command
        try:

            if(command=="addTeacher"):
                teacher = escape(request.form["teacher"])
            ...
            return redirect("/connect/teachers?e=Error")
        try:
            if(command=="outStudent"):
            ...
            return redirect("/connect/students?e=Error")
...
def studentsPage():
    if(session.get('loggedin')):
        try:
            ...
            if(len(e)<1):
                e=""
            return render_template("connect/students.html",teachers=getStudents(),error=e)
        except:
